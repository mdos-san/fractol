...
/*   Updated: 2016/02/15 08:57:28 by mdos-san         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	zoom_out(t_env *env)
{
	...
	while (y < max_y)
	{
		while (x < max_x)
		{
			*(unsigned int *)(new_img.data + new_img.bpp * (x + WIDTH / 4) + new_img.sl * (y + HEIGHT / 4)) = *(unsigned int*)(env->img.data + env->img.bpp * (x * 2) + env->img.sl * (y * 2));
			x++;
		}
		x = 0;
		y++;
