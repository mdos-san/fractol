...
/*   Updated: 2016/02/16 10:20:24 by mdos-san         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
	...
	while (++y < HEIGHT)
	{
		while (++x < WIDTH)
		{
			if (0 < x + mx && x + mx < WIDTH && 0 < y + my && y + my < HEIGHT)
				*(unsigned int *)(new_img.data + new_img.bpp * (x + mx) +
					new_img.sl * (y + my)) = *(unsigned int*)(env->img.data
					+ env->img.bpp * x + env->img.sl * y);
		}
		x = -1;
	}
